...
public enum WorldScanner implements IWorldScanner {
    ...
    @Override
    public List<BlockPos> scanChunkRadius(IPlayerContext ctx, BlockOptionalMetaLookup filter, int max, int yLevelThreshold, int maxSearchRadius,List<ChunkInformation> allblocks ) {
        ArrayList<BlockPos> res = new ArrayList<>();
        ...
        while (true) {
            ...
            for (int xoff = -searchRadiusSq; xoff <= searchRadiusSq; xoff++) {
                for (int zoff = -searchRadiusSq; zoff <= searchRadiusSq; zoff++) {
                    ...
                    allUnloaded = false;
                    if (scanChunkInto(chunkX << 4, chunkZ << 4, chunk, filter, res, max, yLevelThreshold, playerY, coordinateIterationOrder, allblocks)) {
                        foundWithinY = true;
                    }
                }
            }
    ...
    private boolean scanChunkInto(int chunkX, int chunkZ, Chunk chunk, BlockOptionalMetaLookup filter, Collection<BlockPos> result, int max, int yLevelThreshold, int playerY, int[] coordinateIterationOrder, List<ChunkInformation> allblocks) {
        ...
        for (int yIndex = 0; yIndex < 16; yIndex++) {
            ...
            for (int i = 0; i < imax; i++) {
                ...
                int y = yReal | ((i >> 8) & 15);
                BlockPos blockpos = new BlockPos(chunkX | (i & 15), y, chunkZ | ((i >> 4) & 15));
                if (filter.has(state)) {
                    if (result.size() >= max) {
                ...
                allblocks.add(new ChunkInformation(blockpos, state));
